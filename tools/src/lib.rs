...
pub fn read<T: Copy + PartialOrd + std::fmt::Display + std::str::FromStr>(
    ...
    if let Some(v) = token {
        if let Ok(v) = v.parse::<T>() {
            if v < lb || ub < v {
                Err(format!("[WA] Out of range: {}", v))
            } else {
                Ok(v)
            }
        } else {
            Err(format!("[WA] Parse error: {}", v))
        }
    } else {
        Err(format!("[WA] Unexpected end of line"))
    }
}
...
pub fn validate_measure(line: &str, l: usize, n: usize) -> Result<MeasureOp, String> {
    ...
    if i == -1 && y == -1 && x == -1 {
        ...
        Err(format!(
            "[WA] Unexpected measure operation: {} {} {}",
            i, y, x
        ))
    } else {
        let i = i as usize;
...
pub fn gen(
    ...
    if !(MIN_L..=MAX_L).contains(&l) {
        return Err(format!("[WA] l must be in range[{MIN_L}, {MAX_L}]"));
    }
    if !(MIN_N..=MAX_N).contains(&n) {
        return Err(format!("[WA] n must be in range[{MIN_N}, {MAX_N}]"));
    }
    if !(MIN_S..=MAX_S).contains(&s) {
        return Err(format!("[WA] s must be in range[{MIN_S}, {MAX_S}]"));
    }
...
impl Sim {
    ...
    pub fn measure(&mut self, op: &MeasureOp) -> Result<(usize, usize), String> {
        assert!(!op.end, "[WA] This statement must not be reached");

        self.measure_cnt += 1;
        if self.measure_cnt > MAX_MEASURE_CNT {
            return Err(format!("[WA] Too many measure"));
        }
        ...
        let sample = (self.p[r][c] as i64)
            ...
            .ok_or_else(|| {
                format!(
                    "[WA] noise {} is too big. The value is {}",
                    self.measure_cnt - 1,
...
pub fn get_line(lines: &mut Lines) -> Result<String, String> {
    loop {
        let line = lines
            .next()
            .ok_or_else(|| format!("[WA] Output has ended unexpectedly"))?;
...
pub fn parse_row(out: &str, l: usize) -> Result<Vec<usize>, String> {
    ...
    if row.len() != l {
        return Err(format!(
            "[WA] A single row must contain exactly L (= {}) elements but actually has {} elements.",
            l,
            row.len()
        ));
    }
        ...
        match elm {
            Ok(val) => {
                if !(MIN_P <= *val && *val <= MAX_P) {
                    return Err(format!(
                        "[WA] {}-th element: Values of P must be between {} and {}.",
                        j, MIN_P, MAX_P
                    ));
                }
                res.push(*val);
            ...
            Err(err) => {
                return Err(format!("[WA] {}-th element: {}", j, err));
            }
        }
    }
    Ok(res)
...
pub fn parse_arrange(lines: &mut Lines, l: usize) -> Result<ArrangeOp, String> {
    ...
    for i in 0..l {
        ...
        let row = row.map_err(|err| format!("[WA] {}-th row: {}", i, err))?;
        p.push(row);
    }
    Ok(ArrangeOp { p })
}
...
pub fn parse_answer(lines: &mut Lines, n: usize) -> Result<AnswerOp, String> {
    ...
    for i in 0..n {
        ...
        let e = e.map_err(|err| format!("[WA] {}-th answer: {}", i, err))?;
        estimate.push(e);
    }
...
pub fn parse_visualize_data(input: &str, output: &str) -> Result<VisualizeData, String> {
    ...
    let arrange_op = arrange_op.map_err(|err| format!("[WA] Placement: {}", err))?;
    // parse measure
    let mut measure_ops = Vec::new();
    loop {
        let measure_op = parse_measure(&mut lines, input.n, input.l);
        let measure_op = measure_op.map_err(|err| {
            format!(
                "[WA] Measurement: {}-th measure: {}",
                measure_ops.len(),
                err
            )
        })?;
        ...
        if measure_ops.len() > MAX_MEASURE_CNT {
            return Err(format!("[WA] Measurement: Too many measure"));
        }
    }
    // parse answer
    let answer_op = parse_answer(&mut lines, input.n);
    let answer_op = answer_op.map_err(|err| format!("[WA] Answer: {}", err))?;
